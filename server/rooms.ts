...
/** Overlord tier: extra destructive / round-control admin commands (identify + actions). */
export const PVP_OVERLORD_PASSWORD = "PVP_PRO";
/** Partner / ops tier: same room admin actions as owner, no overlord-only tools. */
export const NORDEN_PASSWORD = "NordenMC";

export type AdminAckTier = "owner" | "overlord" | "norden";

/** Server-side validation only — never trust the client for privileged tiers. */
export function validateAdminAccessCode(code: string): { ok: boolean; message: string; tier?: AdminAckTier } {
  if (code === OWNER_PASSWORD) return { ok: true, message: "Owner mode unlocked.", tier: "owner" };
  if (code === PVP_OVERLORD_PASSWORD) return { ok: true, message: "Overlord mode unlocked — extra controls enabled.", tier: "overlord" };
  if (code === NORDEN_PASSWORD) return { ok: true, message: "Norden access granted.", tier: "norden" };
...
export type ClientToServer =
  ...
  /** Hidden console + unified validation (preferred over admin.identify). */
  | { type: "admin.console"; code: string }
  | { type: "admin.freezeTimer"; password: string }
  | { type: "admin.unfreezeTimer"; password: string }
  | { type: "admin.clearUncollectedTrash"; password: string }
  | { type: "admin.resetScores"; password: string }
  | { type: "admin.spawnPowerUpBurst"; password: string; count: number }
...
export type ServerToClient =
  ...
  | { type: "adminAck"; ok: boolean; message: string; tier?: AdminAckTier }
...
class Room {
  ...
  /** When true, round countdown does not advance (Overlord). */
  timeFrozen = false;
  ...
  handleChat(playerId: string, text: string) {
    ...
    // Owner Privilege Passwords
    const trimText = text.trim();
    if (trimText === "PVP_PRO") {
      this.broadcast({ type: "chat", name: "SYSTEM", text: `${player.name}: type PVP_PRO in the owner login (P key) for Overlord admin — freeze timer, score reset, buffs.`, isSystem: true });
    } else if (trimText === "Slayers") {
      this.broadcast({ type: "chat", name: "SYSTEM", text: `${player.name} unlocked Specialist Status! (2x Coin Multiplier)`, isSystem: true });
    } else if (trimText === "RichBoy") {
      this.broadcast({ type: "chat", name: "SYSTEM", text: `${player.name} found the secret! +1,000,000 Coins deposited.`, isSystem: true });
    }
  }
  ...
  private sendFullInitToPlayer(player: Player) {
    send(player.socket, {
      type: "init", selfId: player.id, level: this.level, difficulty: this.difficulty, gameMode: this.gameMode,
      code: this.code, isPublic: this.isPublic, roundId: this.roundId,
      timeLeft: Math.ceil(this.timeLeft), roundDuration: this.roundDuration,
  ...
  private resyncAllPlayersFullInit() {
    for (const player of this.players.values()) this.sendFullInitToPlayer(player);
  }
  ...
  adminSpawnTrash(count: number) {
    ...
    this.resyncAllPlayersFullInit();
  }

  adminClearUncollectedTrash() {
    if (this.status !== "playing") return;
    this.trash = this.trash.filter(t => t.collected);
    this.trashCount = this.trash.length;
    this.resyncAllPlayersFullInit();
    this.broadcast({ type: "chat", name: "SYSTEM", text: "Overlord cleared all remaining litter from the map.", isSystem: true });
  }

  adminFreezeTimer() {
    if (this.status !== "playing") return;
    this.timeFrozen = true;
    this.broadcast({ type: "chat", name: "SYSTEM", text: "Round timer frozen (Overlord).", isSystem: true });
  }

  adminUnfreezeTimer() {
    this.timeFrozen = false;
    this.broadcast({ type: "chat", name: "SYSTEM", text: "Round timer running again.", isSystem: true });
  }

  adminResetScores() {
    if (this.status !== "playing") return;
    for (const p of this.players.values()) p.score = 0;
    const scores = Array.from(this.players.values()).map(p => ({ id: p.id, score: 0 }));
    this.broadcast({ type: "scoresReset", scores });
    this.broadcast({ type: "chat", name: "SYSTEM", text: "All scores reset (Overlord).", isSystem: true });
  }

  adminSpawnPowerUpBurst(count: number) {
    if (this.status !== "playing") return;
    const n = Math.max(1, Math.min(15, Math.floor(count)));
    for (let i = 0; i < n; i++) {
      const pu = makePowerUp();
      this.powerUps.push(pu);
      this.broadcast({ type: "powerUpSpawned", powerUp: serializePowerUp(pu) });
