...
export function startGame(mount: HTMLElement, onState: (state: GameState) => void): GameHandle {
  ...
  function updateLocalMovement(dt: number) {
    ...
    const desiredX = Math.max(-WORLD_HALF, Math.min(WORLD_HALF, player.position.x + moveX * speed * dt));
    const desiredZ = Math.max(-WORLD_HALF, Math.min(WORLD_HALF, player.position.z + moveZ * speed * dt));
