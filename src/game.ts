...
export type Level = "park" | "beach" | "city" | "nightcity" | "arctic" | "jungle" | "cybercity";
export type GameMode = "speedrun" | "percent100" | "powerup" | "simulator";
export type Difficulty = "easy" | "normal" | "hard";
export type GameStatus =
  | "menu" | "connecting" | "playing" | "waiting" | "disconnected" | "unsupported";
export type TrashType = "plastic" | "ewaste" | "organic";
...
export const GAME_MODE_DESCRIPTIONS: Record<GameMode, string> = {
  ...
  simulator: "Deep simulation mode with economy, upgrades, and environmental impact tracking.",
};
...
export type AdminCommand =
  ...
  | { kind: "setInfiniteStorage"; enabled: boolean }
  | { kind: "setSpeedMultiplier"; multiplier: number }
  | { kind: "setCoinMultiplier"; multiplier: number }
  | { kind: "enableExecutionerVacuum"; enabled: boolean }
  | { kind: "setUnlimitedAura"; enabled: boolean }
  | { kind: "teleportAll"; x: number; z: number }
  | { kind: "clearAllTrash" }
  | { kind: "spawnMinkDrones"; count: number }
  /** Overlord (PVP_PRO) — server + UI gated */
  | { kind: "freezeTimer" }
  | { kind: "unfreezeTimer" }
  | { kind: "clearUncollectedTrash" }
  | { kind: "resetScores" }
  | { kind: "spawnPowerUpBurst"; count: number };

export interface MiniMapData {
  worldHalf: number;
  selfX: number;
  selfZ: number;
  /** Player body yaw (rad); map rotates so forward is up. */
  facingYaw: number;
  level: Level;
  magnetActive: boolean;
  speedActive: boolean;
  vacuumActive: boolean;
  magnetRadiusWorld: number;
  collectRadiusWorld: number;
...
export interface GameState {
  ...
  miniMap: MiniMapData;
  isOwner: boolean;
  /** `overlord` unlocks extra server commands (password PVP_PRO). `norden` = NordenMC ops tier. */
  adminTier: "none" | "owner" | "overlord" | "norden";
  ownerStatus: { ok: boolean; message: string } | null; godMode: boolean;
  stats: SessionStats;
  isPaused: boolean;
  chatMessages: { id: number; name: string; text: string; isSystem: boolean }[];
  // PVP_PRO powers
  infiniteStorage: boolean;
  speedMultiplier: number;
  coinMultiplier: number;
  executionerVacuum: boolean;
  unlimitedAura: boolean;
  minkDrones: number;
}
...
export interface GameHandle {
  ...
  /** Hidden console — sends `admin.console` for server validation (same ack as owner login). */
  submitAdminConsole: (code: string) => void;
...
const FACTS: Record<TrashType, string[]> = {
  ...
  ewaste: [
    "E-waste is the fastest-growing waste stream globally, growing 5x faster than other waste.",
    "Only 20% of e-waste is formally recycled; the rest ends up in landfills or incinerators.",
    "One smartphone contains over 60 different elements, many of which are toxic if not recycled properly.",
  ],
  organic: [
...
const TRASH_PARTICLE_COLORS: Record<TrashType, number> = {
  plastic: 0x88ccee, ewaste: 0xff6666, organic: 0x66cc66,
};
...
type ServerToClient =
  ...
  | { type: "adminAck"; ok: boolean; message: string; tier?: "owner" | "overlord" | "norden" }
  | { type: "scoresReset"; scores: { id: string; score: number }[] }
  | { type: "error"; message: string }
  | { type: "chat"; name: string; text: string; isSystem?: boolean };
...
export function startGame(mount: HTMLElement, onState: (state: GameState) => void): GameHandle {
  ...
  try {
    ...
    onState({
      status: "unsupported", level: "park", gameMode: "speedrun", difficulty: "normal", roundDuration: 120,
      ...
      miniMap: {
        worldHalf: WORLD_HALF, selfX: 0, selfZ: 0, facingYaw: 0, level: "park",
        magnetActive: false, speedActive: false, vacuumActive: false,
        magnetRadiusWorld: MAGNET_RADIUS, collectRadiusWorld: COLLECT_RADIUS,
        others: [], trash: [], powerUps: [],
      },
      isOwner: false, adminTier: "none", ownerStatus: null, godMode: false, stats: { trashCleaned: 0, powerUpsCollected: 0, bestCombo: 0, pointsEarned: 0 },
      isPaused: false, chatMessages: [],
      infiniteStorage: false, speedMultiplier: 1, coinMultiplier: 1, executionerVacuum: false, unlimitedAura: false, minkDrones: 0
    });
    return {
      ...
      setMouseSensitivity: () => undefined, submitOwnerPassword: () => undefined, submitAdminConsole: () => undefined,
      sendAdmin: () => undefined, setGodMode: () => undefined, setPaused: () => undefined, sendChat: () => undefined, dispose: () => undefined,
    };
  }
  ...
  let chatMessages: { id: number; name: string; text: string; isSystem: boolean }[] = [];
  let nextChatId = 1;
  let magnetMs = 0;
  let speedMs = 0;
  let isOwner = false;
  let adminTier: "none" | "owner" | "overlord" | "norden" = "none";
  ...
  // PVP_PRO powers state
  let infiniteStorage = false;
  let speedMultiplier = 1;
  let coinMultiplier = 1;
  let executionerVacuum = false;
  let unlimitedAura = false;
  let minkDrones = 0;
  ...
  const pet = createPet(); scene.add(pet);
  let petBobPhase = 0;
  ...
  function startMusic() {
    ...
    const cyberNotes = [130, 155, 185, 155];
    const noteMap: Record<Level, number[]> = { park: parkNotes, beach: beachNotes, city: cityNotes, nightcity: nightNotes, arctic: arcticNotes, jungle: jungleNotes, cybercity: cyberNotes };
    const baseNotes = noteMap[level];
    const nd = level === "nightcity" ? 0.6 : level === "city" ? 0.75 : level === "cybercity" ? 0.5 : 0.9;
    ...
    if (level === "cybercity") startLoop(baseNotes.map(n => n * 0.25), "sawtooth", 0.4);
  }
  function stopMusic() {
  ...
  function buildMiniMap(): MiniMapData {
    ...
    const collectR = executionerVacuum ? COLLECT_RADIUS * 3 : COLLECT_RADIUS;
    return {
      worldHalf: WORLD_HALF,
      selfX: player.position.x,
      selfZ: player.position.z,
      facingYaw: player.rotation.y,
      level,
      magnetActive: magnetMs > 0,
      speedActive: speedMs > 0,
      vacuumActive: executionerVacuum,
      magnetRadiusWorld: MAGNET_RADIUS,
      collectRadiusWorld: collectR,
      others, trash, powerUps,
    };
  }
  ...
  function emitState() {
    onState({
      ...
      isOwner, adminTier, ownerStatus, godMode, stats: { ...stats }, isPaused, chatMessages,
      infiniteStorage, speedMultiplier, coinMultiplier, executionerVacuum, unlimitedAura, minkDrones
    });
  }
  ...
  function setLevelAtmosphere(lvl: Level) {
    ...
    } else if (lvl === "cybercity") {
      scene.background = new THREE.Color(0x0a0a1a); scene.fog = new THREE.Fog(0x0a0a1a, 15, 60);
      hemiLight.color.set(0x00ffff); hemiLight.groundColor.set(0x1a0030);
      sun.color.set(0xff00ff); sun.intensity = 0.3;
    } else {
  ...
  function buildLevel(lvl: Level) {
    ...
    const groundColor = lvl === "beach" ? 0xf2d8a0 : lvl === "city" ? 0x6b6f72 : lvl === "nightcity" ? 0x111118 : lvl === "arctic" ? 0xe8f4f8 : lvl === "jungle" ? 0x2d4a1e : lvl === "cybercity" ? 0x1a1a2e : 0x6cba5b;
    ...
    if (lvl === "nightcity") {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        const pl = new THREE.PointLight(col, 2, 15); pl.position.set(lx, 3, lz); sceneryGroup.add(pl);
      }
      // Floating neon signs
      for (let i = 0; i < 12; i++) {
        const sign = createNeonSign(rand);
        const sx = (rand() * 2 - 1) * 18; const sz = (rand() * 2 - 1) * 18;
        sign.position.set(sx, 4 + rand() * 3, sz); sceneryGroup.add(sign);
      }
    }
    ...
    if (lvl === "park") {
      ...
      for (let i = 0; i < 5; i++) { const c = createCabin(); placeRand(c, 6, rand, 1.4); sceneryGroup.add(c); }
      for (let i = 0; i < 12; i++) { const b = createBench(rand); placeRand(b, 3, rand, 1.0); sceneryGroup.add(b); }
      // Enhanced flowers
      for (let i = 0; i < 40; i++) {
        const fx = (rand() * 2 - 1) * WORLD_HALF; const fz = (rand() * 2 - 1) * WORLD_HALF;
        const cols = [0xff6b6b, 0xffd700, 0xff69b4, 0x9b59b6, 0x00bcd4, 0xff5722];
      ...
      // Pathways
      const pathMat = new THREE.MeshStandardMaterial({ color: 0xd4a574, roughness: 0.9 });
      for (let i = 0; i < 8; i++) {
        const path = new THREE.Mesh(new THREE.PlaneGeometry(8 + rand() * 4, 1.5), pathMat);
        path.rotation.x = -Math.PI / 2;
        const angle = (i / 8) * Math.PI * 2;
        path.position.set(Math.cos(angle) * 12, 0.01, Math.sin(angle) * 12);
        path.rotation.y = angle + Math.PI / 2;
        sceneryGroup.add(path);
      }
      // Fountain
      const fountainBase = new THREE.Mesh(new THREE.CylinderGeometry(2, 2.5, 0.5, 16), new THREE.MeshStandardMaterial({ color: 0x888888, roughness: 0.3 }));
      fountainBase.position.set(0, 0.25, 0); sceneryGroup.add(fountainBase);
      const water = new THREE.Mesh(new THREE.CylinderGeometry(1.8, 1.8, 0.3, 16), new THREE.MeshStandardMaterial({ color: 0x4fc3f7, transparent: true, opacity: 0.7, roughness: 0.1 }));
      water.position.set(0, 0.4, 0); sceneryGroup.add(water);
      // Lamp posts
      for (let i = 0; i < 8; i++) {
        const angle = (i / 8) * Math.PI * 2;
        const lamp = createStreetLamp(0xfff9c4);
        lamp.position.set(Math.cos(angle) * 15, 0, Math.sin(angle) * 15);
        sceneryGroup.add(lamp);
      }
    }

    if (lvl === "beach") {
      for (let i = 0; i < 18; i++) { const t = createPalmTree(); placeRand(t, 4, rand, 0.45); sceneryGroup.add(t); }
      // Beach umbrellas
      for (let i = 0; i < 8; i++) {
        const ux = (rand() * 2 - 1) * 20; const uz = (rand() * 2 - 1) * 20;
        const umbrella = createBeachUmbrella();
        umbrella.position.set(ux, 0, uz); sceneryGroup.add(umbrella);
      }
      // Seashells
      for (let i = 0; i < 20; i++) {
        const sx = (rand() * 2 - 1) * 25; const sz = (rand() * 2 - 1) * 25;
        const shell = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.15, 8), new THREE.MeshStandardMaterial({ color: 0xfff8dc, roughness: 0.8 }));
        shell.position.set(sx, 0.05, sz); shell.rotation.x = Math.PI; sceneryGroup.add(shell);
      }
      // Beach towels
      for (let i = 0; i < 6; i++) {
        const tx = (rand() * 2 - 1) * 18; const tz = (rand() * 2 - 1) * 18;
        const towelColors = [0xff6b6b, 0x4ecdc4, 0xffd93d, 0x6c5ce7];
        const towel = new THREE.Mesh(new THREE.PlaneGeometry(2, 1), new THREE.MeshStandardMaterial({ color: towelColors[Math.floor(rand() * towelColors.length)], roughness: 0.9 }));
        towel.rotation.x = -Math.PI / 2; towel.position.set(tx, 0.02, tz); towel.rotation.z = rand() * Math.PI; sceneryGroup.add(towel);
      }
    }

    if (lvl === "city") {
      for (let i = 0; i < 20; i++) { const b = createBuilding(rand); placeRand(b.mesh, 6, rand, b.radius); sceneryGroup.add(b.mesh); }
      for (let i = 0; i < 12; i++) { const b = createBench(rand); placeRand(b, 3, rand, 1.0); sceneryGroup.add(b); }
      const lampPos: [number, number][] = [[-12, -12], [12, -12], [-12, 12], [12, 12], [-20, 0], [20, 0], [0, -20], [0, 20], [-8, -8], [8, 8], [-8, 8], [8, -8]];
      ...
      for (let i = 0; i < 10; i++) {
        const tx = (rand() * 2 - 1) * 18; const tz = (rand() * 2 - 1) * 18;
        const trashCan = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.35, 0.8, 12), new THREE.MeshStandardMaterial({ color: 0x2e7d32, metalness: 0.6 }));
        trashCan.position.set(tx, 0.4, tz); sceneryGroup.add(trashCan);
      }
      // Cars
      for (let i = 0; i < 6; i++) {
        const car = createCar(rand);
        const cx = (rand() * 2 - 1) * 15; const cz = (rand() * 2 - 1) * 15;
        car.position.set(cx, 0, cz); car.rotation.y = rand() * Math.PI * 2; sceneryGroup.add(car);
      }
    }

    if (lvl === "cybercity") {
      // Neon grid ground
      const gridMat = new THREE.MeshStandardMaterial({ color: 0x00ffff, emissive: 0x00ffff, emissiveIntensity: 0.5, transparent: true, opacity: 0.3 });
      for (let i = -25; i <= 25; i += 5) {
        const lineH = new THREE.Mesh(new THREE.PlaneGeometry(120, 0.1), gridMat);
        lineH.rotation.x = -Math.PI / 2; lineH.position.set(0, 0.01, i); sceneryGroup.add(lineH);
        const lineV = new THREE.Mesh(new THREE.PlaneGeometry(0.1, 120), gridMat);
        lineV.rotation.x = -Math.PI / 2; lineV.position.set(i, 0.01, 0); sceneryGroup.add(lineV);
      }
      // Massive cyber towers
      for (let i = 0; i < 20; i++) {
        const cb = createCyberTower(rand); placeRand(cb.mesh, 8, rand, cb.radius); sceneryGroup.add(cb.mesh);
      }
      // Holographic billboards
      const holoColors = [0x00ffff, 0xff00ff, 0x00ff00, 0xffff00];
      for (let i = 0; i < 12; i++) {
        const hx = (rand() * 2 - 1) * 20; const hz = (rand() * 2 - 1) * 20;
        const holo = createHologram(holoColors[Math.floor(rand() * holoColors.length)]);
        holo.position.set(hx, 3 + rand() * 4, hz); sceneryGroup.add(holo);
      }
      // Floating data streams
      for (let i = 0; i < 15; i++) {
        const ds = createDataStream(); ds.position.set((rand() * 2 - 1) * 22, 2 + rand() * 8, (rand() * 2 - 1) * 22);
        sceneryGroup.add(ds);
      }
      // Neon point lights everywhere
      const nlColors = [0x00ffff, 0xff00ff, 0x00ff88, 0xff6600, 0xffff00];
      for (let i = 0; i < 25; i++) {
        const col = nlColors[i % nlColors.length];
        const lx = (rand() * 2 - 1) * 24; const lz = (rand() * 2 - 1) * 24;
        const pl = new THREE.PointLight(col, 2, 18); pl.position.set(lx, 5 + rand() * 5, lz); sceneryGroup.add(pl);
      }
    }
  }
  ...
  function createPet(): THREE.Group {
    const g = new THREE.Group();
    const body = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.25, 0.4), new THREE.MeshStandardMaterial({ color: 0x4fc3f7, metalness: 0.6, roughness: 0.2 }));
    body.castShadow = true; g.add(body);
    const eye = new THREE.Mesh(new THREE.PlaneGeometry(0.2, 0.08), new THREE.MeshBasicMaterial({ color: 0xffffff }));
    eye.position.set(0, 0.05, 0.201); g.add(eye);
    const pupil = new THREE.Mesh(new THREE.PlaneGeometry(0.06, 0.06), new THREE.MeshBasicMaterial({ color: 0x000000 }));
    pupil.position.set(0, 0.05, 0.202); g.add(pupil);
    return g;
  }
  ...
  function createCyberTower(rng: () => number): { mesh: THREE.Group; radius: number } {
    const g = new THREE.Group();
    const w = 2 + rng() * 3; const d = 2 + rng() * 3; const h = 8 + rng() * 15;
    ...
    const neonCols = [0x00ffff, 0xff00ff, 0x00ff88, 0xff6600];
    for (let i = 0; i < Math.floor(h / 2); i++) {
      const col = neonCols[Math.floor(rng() * neonCols.length)];
      const strip = new THREE.Mesh(new THREE.BoxGeometry(w + 0.1, 0.15, d + 0.1), new THREE.MeshStandardMaterial({ color: col, emissive: col, emissiveIntensity: 3 }));
      strip.position.set(0, 1 + i * 2, 0); g.add(strip);
    }
    // Glowing windows
    const winMat = new THREE.MeshStandardMaterial({ color: 0x00ffff, emissive: 0x00ffff, emissiveIntensity: 1.5 });
    const rows = Math.floor(h / 1.5); const wc = Math.floor(w / 0.8);
    for (let r = 0; r < rows; r++) for (let c = 0; c < wc; c++) if (rng() < 0.4) {
      const win = new THREE.Mesh(new THREE.PlaneGeometry(0.35, 0.5), winMat);
      win.position.set(-w / 2 + 0.4 + c * (w / wc), 0.7 + r * 1.5, d / 2 + 0.01); g.add(win);
    }
    // Top antenna
    const ant = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 3 + rng() * 2, 8), new THREE.MeshStandardMaterial({ color: 0x888888, metalness: 0.9 }));
    ant.position.set(0, h + 1.5, 0); g.add(ant);
    const antLight = new THREE.PointLight(0xff0000, 1, 8);
    antLight.position.set(0, h + 3, 0); g.add(antLight);
    colliders.push({ x: 0, z: 0, r: Math.max(w, d) * 0.5 + 0.3 });
    return { mesh: g, radius: Math.max(w, d) * 0.5 + 0.3 };
  }
  function createHologram(color: number): THREE.Group {
    const g = new THREE.Group();
    const mat = new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 2, transparent: true, opacity: 0.6, side: THREE.DoubleSide });
    const panel = new THREE.Mesh(new THREE.PlaneGeometry(3, 2), mat);
    panel.position.y = 1; g.add(panel);
    // Scanning line effect
    const scanLine = new THREE.Mesh(new THREE.PlaneGeometry(3, 0.1), new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.8 }));
    scanLine.position.y = 1; g.add(scanLine);
    return g;
  }
  function createDataStream(): THREE.Group {
    const g = new THREE.Group();
    const mat = new THREE.MeshBasicMaterial({ color: 0x00ffff, transparent: true, opacity: 0.5 });
    for (let i = 0; i < 5; i++) {
      const bit = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.1, 0.1), mat);
      bit.position.set(0, i * 0.3, 0); g.add(bit);
    }
    return g;
  }
  function createBeachUmbrella(): THREE.Group {
    const g = new THREE.Group();
    const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 2, 8), new THREE.MeshStandardMaterial({ color: 0x8b4513 }));
    pole.position.y = 1; g.add(pole);
    const canopyColors = [0xff6b6b, 0x4ecdc4, 0xffd93d, 0x6c5ce7, 0xff9ff3];
    const canopyColor = canopyColors[Math.floor(Math.random() * canopyColors.length)];
    const canopy = new THREE.Mesh(new THREE.ConeGeometry(1.5, 0.8, 8), new THREE.MeshStandardMaterial({ color: canopyColor, side: THREE.DoubleSide }));
    canopy.position.y = 2; g.add(canopy);
    return g;
  }
  function createCar(rng: () => number): THREE.Group {
    const g = new THREE.Group();
    const carColors = [0xe74c3c, 0x3498db, 0x2ecc71, 0xf1c40f, 0x9b59b6, 0x1abc9c];
    const bodyColor = carColors[Math.floor(rng() * carColors.length)];
    const body = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.6, 0.9), new THREE.MeshStandardMaterial({ color: bodyColor, metalness: 0.8, roughness: 0.2 }));
    body.position.y = 0.5; body.castShadow = true; g.add(body);
    const roof = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.5, 0.8), new THREE.MeshStandardMaterial({ color: bodyColor, metalness: 0.8, roughness: 0.2 }));
    roof.position.set(0, 0.9, 0); roof.castShadow = true; g.add(roof);
    const wheelMat = new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.9 });
    const wheelPositions = [[-0.6, 0.3], [0.6, 0.3], [-0.6, -0.3], [0.6, -0.3]];
    for (const [wx, wz] of wheelPositions) {
      const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.1, 16), wheelMat);
      wheel.rotation.z = Math.PI / 2; wheel.position.set(wx, 0.2, wz); g.add(wheel);
    }
    return g;
  }
  function createNeonSign(rng: () => number): THREE.Group {
    const g = new THREE.Group();
    const neonColors = [0xff00ff, 0x00ffff, 0xff6600, 0x00ff88, 0xffff00];
    const col = neonColors[Math.floor(rng() * neonColors.length)];
    const frame = new THREE.Mesh(new THREE.BoxGeometry(2, 1, 0.1), new THREE.MeshStandardMaterial({ color: 0x1a1a1a, metalness: 0.8 }));
    frame.position.y = 0.5; g.add(frame);
    const glow = new THREE.Mesh(new THREE.PlaneGeometry(1.8, 0.8), new THREE.MeshStandardMaterial({ color: col, emissive: col, emissiveIntensity: 3, side: THREE.DoubleSide }));
    glow.position.set(0, 0.5, 0.06); g.add(glow);
    return g;
  }
  ...
  function createTrash(type: TrashType): THREE.Group {
    ...
    if (type === "plastic") {
      ...
      const board = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.05, 0.25), new THREE.MeshStandardMaterial({ color: 0x2a5a2a, metalness: 0.8, roughness: 0.3 })); board.position.y = 0.15; board.castShadow = true; g.add(board);
      const chip = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.03, 0.08), new THREE.MeshStandardMaterial({ color: 0x1a1a1a, metalness: 0.9 })); chip.position.set(0.05, 0.18, 0); g.add(chip);
      const wire = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.2, 8), new THREE.MeshStandardMaterial({ color: 0xff6600, metalness: 0.7 })); wire.position.set(-0.1, 0.15, 0); wire.rotation.z = Math.PI / 4; g.add(wire);
    } else {
    ...
    const gCol = type === "plastic" ? 0x44aaff : type === "ewaste" ? 0xff4444 : 0x66cc66;
  ...
  function handleServerMessage(msg: ServerToClient) {
    switch (msg.type) {
      ...
      case "adminAck": {
        ownerStatus = { ok: msg.ok, message: msg.message };
        if (msg.ok) {
          isOwner = true;
          if (msg.tier === "overlord") adminTier = "overlord";
          else if (msg.tier === "norden") adminTier = "norden";
          else adminTier = "owner";
        } else {
          ownerPassword = null; isOwner = false; adminTier = "none";
        }
        emitState();
        break;
      }
      case "scoresReset": {
        players = players.map(p => {
          const u = msg.scores.find(s => s.id === p.id);
          return u ? { ...p, score: u.score } : p;
        });
        const me = players.find(p => p.id === selfId);
        if (me) selfScore = me.score;
        stats.pointsEarned = selfScore;
        emitState();
        break;
      }
      case "error": { errorMessage = msg.message; status = "menu"; emitState(); break; }
      case "chat": {
        chatMessages = [...chatMessages, { id: nextChatId++, name: msg.name, text: msg.text, isSystem: !!msg.isSystem }];
  ...
  function submitAdminConsole(code: string) {
    const c = code.trim().slice(0, 64);
    if (!c) return;
    ownerPassword = c;
    send({ type: "admin.console", code: c });
  }
  function submitOwnerPassword(password: string) { submitAdminConsole(password); }
  function sendAdmin(cmd: AdminCommand) {
    ...
    switch (cmd.kind) {
      ...
      case "setInfiniteStorage":
      case "setSpeedMultiplier":
      case "setCoinMultiplier":
      case "enableExecutionerVacuum":
      case "setUnlimitedAura":
      case "teleportAll":
      case "clearAllTrash":
      case "spawnMinkDrones":
        if (adminTier !== "overlord") return;
        if (cmd.kind === "setInfiniteStorage") infiniteStorage = cmd.enabled;
        else if (cmd.kind === "setSpeedMultiplier") speedMultiplier = cmd.multiplier;
        else if (cmd.kind === "setCoinMultiplier") coinMultiplier = cmd.multiplier;
        else if (cmd.kind === "enableExecutionerVacuum") executionerVacuum = cmd.enabled;
        else if (cmd.kind === "setUnlimitedAura") unlimitedAura = cmd.enabled;
        else if (cmd.kind === "teleportAll") player.position.set(cmd.x, 0, cmd.z);
        else if (cmd.kind === "clearAllTrash") {
          trashItems.forEach(item => { scene.remove(item.mesh); disposeObject(item.mesh); });
          trashItems.clear();
        } else if (cmd.kind === "spawnMinkDrones") minkDrones = cmd.count;
        emitState();
        break;
      case "freezeTimer":
      case "unfreezeTimer":
      case "clearUncollectedTrash":
      case "resetScores":
      case "spawnPowerUpBurst":
        if (adminTier !== "overlord" || !ownerPassword) return;
        if (cmd.kind === "freezeTimer") send({ type: "admin.freezeTimer", password: ownerPassword });
        else if (cmd.kind === "unfreezeTimer") send({ type: "admin.unfreezeTimer", password: ownerPassword });
        else if (cmd.kind === "clearUncollectedTrash") send({ type: "admin.clearUncollectedTrash", password: ownerPassword });
        else if (cmd.kind === "resetScores") send({ type: "admin.resetScores", password: ownerPassword });
        else send({ type: "admin.spawnPowerUpBurst", password: ownerPassword, count: cmd.count });
        break;
    }
  }
  ...
  function backToMenu() {
    ...
    isSprinting = false; magnetMs = 0; speedMs = 0; isOwner = false; adminTier = "none"; ownerPassword = null; ownerStatus = null;
  ...
  function animate() {
    ...
    if ((status === "playing" || status === "waiting") && !isPaused) {
      ...
      if (status === "playing" || status === "waiting") {
        pet.visible = true;
        petBobPhase += dt * 4;
        const offset = new THREE.Vector3(1.2, 1.4 + Math.sin(petBobPhase) * 0.15, 0.8);
        offset.applyEuler(new THREE.Euler(0, player.rotation.y, 0));
        const targetPos = player.position.clone().add(offset);
        pet.position.lerp(targetPos, Math.min(1, dt * 6));

        let targetRotY = player.rotation.y;
        let dr = targetRotY - pet.rotation.y;
        while (dr > Math.PI) dr -= Math.PI * 2; while (dr < -Math.PI) dr += Math.PI * 2;
        pet.rotation.y += dr * Math.min(1, dt * 4);
      } else {
        pet.visible = false;
      }
    } else {
  ...
  function updateLocalMovement(dt: number) {
    ...
    speed *= speedMultiplier; // Apply PVP_PRO speed multiplier
  ...
  function attemptCollect() {
    const collectRadius = executionerVacuum ? COLLECT_RADIUS * 3 : COLLECT_RADIUS; // Executioner Vacuum triples collection radius
    for (const item of trashItems.values()) {
      ...
      if (dx * dx + dz * dz < (collectRadius + PLAYER_RADIUS) ** 2) { item.pendingCollect = true; send({ type: "collect", trashId: item.id }); }
    }
  }
  ...
  return { start, backToMenu, toggleSound, sendEmote, setMusicVolume, setSfxVolume, setMouseSensitivity, submitOwnerPassword, submitAdminConsole, sendAdmin, setGodMode, setPaused, sendChat, dispose };
}
