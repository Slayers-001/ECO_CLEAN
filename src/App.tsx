...
/** Glassmorphism admin console — codes validated only on the Node WebSocket server (`admin.console`). */
function AdminCommandConsole({
  open,
  onClose,
  onSubmit,
  lastStatus,
  ...
  lastStatus: GameState["ownerStatus"];
}) {
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" aria-modal="true" role="dialog" aria-labelledby="admin-console-title">
      <button
        type="button"
      ...
      />
      <div
        className="relative w-full max-w-md overflow-hidden rounded-3xl border border-white/30 bg-gradient-to-br from-white/20 via-white/12 to-emerald-400/10 px-7 py-7 shadow-[0_28px_64px_rgba(0,0,0,0.5)] ring-1 ring-white/20 backdrop-blur-2xl"
        onClick={e => e.stopPropagation()}
      >
        <div className="pointer-events-none absolute -right-16 -top-16 h-40 w-40 rounded-full bg-cyan-400/20 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-12 -left-12 h-36 w-36 rounded-full bg-emerald-500/15 blur-3xl" />
        <div className="relative">
          <div className="flex items-start justify-between gap-3 mb-5">
            <div>
              <h2 id="admin-console-title" className="text-lg font-bold tracking-tight text-white drop-shadow-sm">
                Admin Command Console
              </h2>
              <p className="mt-1 text-xs text-white/75 leading-relaxed">
                Access codes are validated on the server. Nothing unlocks until <code className="rounded bg-black/25 px-1 py-0.5 text-[10px]">adminAck</code> returns success.
              </p>
              <p className="mt-2 text-[10px] text-white/50 font-mono">Toggle: Ctrl+Shift+`</p>
            </div>
            <button
              type="button"
              onClick={onClose}
              className="shrink-0 rounded-xl border border-white/25 bg-white/10 px-3 py-1.5 text-sm font-semibold text-white hover:bg-white/20"
            >
              Esc
            </button>
          </div>
          ...
          >
            <label className="block text-[11px] font-semibold uppercase tracking-wider text-white/80">Access code</label>
            <input
              ref={inputRef}
              type="password"
              value={line}
              ...
              spellCheck={false}
              maxLength={64}
              placeholder="PVP_PRO · Slayers · NordenMC"
              className="w-full rounded-2xl border border-white/25 bg-black/25 px-4 py-3 text-sm text-white placeholder:text-white/40 shadow-inner outline-none ring-0 focus:border-emerald-300/60 focus:bg-black/35"
            />
            <button
              type="submit"
              className="w-full rounded-2xl border border-white/25 bg-emerald-500/80 py-3 text-sm font-bold text-white shadow-lg backdrop-blur-sm hover:bg-emerald-500"
            >
              Send to server
            </button>
          </form>
...
const MODE_COLORS: Record<GameMode, string> = {
  ...
  simulator: "bg-cyan-600",
};

const POWERUP_KINDS: PowerUpKind[] = ["magnet", "speed", "bonus"];

const ALL_LEVELS: Level[] = ["park", "beach", "city", "nightcity", "arctic", "jungle", "cybercity"];
...
const DEFAULT_STATE: GameState = {
  ...
  miniMap: {
    worldHalf: 28, selfX: 0, selfZ: 0, facingYaw: 0, level: "park",
    magnetActive: false, speedActive: false, vacuumActive: false,
    magnetRadiusWorld: 6, collectRadiusWorld: 1.4,
    others: [], trash: [], powerUps: [],
  },
  isOwner: false, adminTier: "none", ownerStatus: null, godMode: false,
...
/** Tactical radar: forward-up rotation, range rings, zoom, magnet / pickup hints */
function MiniMap({ data, godMode }: { data: MiniMapData; godMode: boolean }) {
  const clipUid = useId().replace(/:/g, "");
  const [zoomIdx, setZoomIdx] = useState(1);
  const zoom = ([0.72, 1, 1.35] as const)[zoomIdx];
  const SIZE = 252;
  const HALF = SIZE / 2;
  const effectiveHalf = data.worldHalf / zoom;
  const scale = HALF / effectiveHalf;
  const projX = (x: number) => HALF + (x - data.selfX) * scale;
  const projZ = (z: number) => HALF + (z - data.selfZ) * scale;
  const rotDeg = (-data.facingYaw * 180) / Math.PI;
  const clipR = HALF - 6;
  const inBounds = (cx: number, cy: number) => {
    const dx = cx - HALF; const dy = cy - HALF;
    return dx * dx + dy * dy <= clipR * clipR;
  };
  return (
    <div className="pointer-events-none select-none">
      <div
        className="relative rounded-2xl shadow-2xl border border-emerald-400/30 overflow-hidden"
        style={{ width: SIZE, height: SIZE, background: "radial-gradient(ellipse at center, rgba(6,40,30,0.92) 0%, rgba(0,0,0,0.94) 70%)" }}
      >
        <svg width={SIZE} height={SIZE} className="block">
          <defs>
            <radialGradient id={`mmGlow-${clipUid}`} cx="50%" cy="50%" r="55%">
              <stop offset="0%" stopColor="rgba(52,211,153,0.12)" />
              <stop offset="100%" stopColor="rgba(0,0,0,0)" />
            </radialGradient>
            <clipPath id={`mmClip-${clipUid}`}>
              <circle cx={HALF} cy={HALF} r={clipR} />
            </clipPath>
          </defs>
          <circle cx={HALF} cy={HALF} r={clipR} fill={`url(#mmGlow-${clipUid})`} />
          <circle cx={HALF} cy={HALF} r={clipR} fill="none" stroke="rgba(52,211,153,0.35)" strokeWidth={1.5} />
          {[0.45, 0.72, 1].map((t, i) => (
            <circle key={i} cx={HALF} cy={HALF} r={clipR * t} fill="none" stroke="rgba(255,255,255,0.06)" strokeWidth={0.75} strokeDasharray={i === 2 ? "none" : "3 4"} />
          ))}
          <text x={HALF} y={11} textAnchor="middle" fontSize="9" fontWeight="700" fill="rgba(52,211,153,0.85)">FWD</text>
          <text x={HALF} y={SIZE - 4} textAnchor="middle" fontSize="7" fill="rgba(255,255,255,0.35)">{LEVEL_LABELS[data.level]}</text>
          <g clipPath={`url(#mmClip-${clipUid})`}>
            <g transform={`translate(${HALF} ${HALF}) rotate(${rotDeg}) translate(${-HALF} ${-HALF})`}>
              <rect
                x={projX(-data.worldHalf)} y={projZ(-data.worldHalf)}
                width={2 * data.worldHalf * scale} height={2 * data.worldHalf * scale}
                fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.18)" strokeWidth={1}
              />
              {[-14, 0, 14].map(v => (
                <g key={v}>
                  <line x1={projX(v)} y1={projZ(-data.worldHalf)} x2={projX(v)} y2={projZ(data.worldHalf)} stroke="rgba(255,255,255,0.05)" strokeWidth={0.5} />
                  <line x1={projX(-data.worldHalf)} y1={projZ(v)} x2={projX(data.worldHalf)} y2={projZ(v)} stroke="rgba(255,255,255,0.05)" strokeWidth={0.5} />
                </g>
              ))}
              {data.trash.map((t, i) => {
                const cx = projX(t.x); const cy = projZ(t.z);
                if (!inBounds(cx, cy)) return null;
                return <circle key={`t${i}`} cx={cx} cy={cy} r={2.8} fill="#fbbf24" opacity={0.92} />;
              })}
              {data.powerUps.map((p, i) => {
                const cx = projX(p.x); const cy = projZ(p.z);
                if (!inBounds(cx, cy)) return null;
                return (
                  <circle
                    key={`p${i}`}
                    cx={cx} cy={cy} r={4.2}
                    fill={p.kind === "magnet" ? "#ef4444" : p.kind === "speed" ? "#facc15" : "#c084fc"}
                    stroke="white" strokeWidth={1} opacity={0.95}
                  />
                );
              })}
              {data.others.map(o => {
                const cx = projX(o.x); const cy = projZ(o.z);
                if (!inBounds(cx, cy)) return null;
                return <circle key={o.id} cx={cx} cy={cy} r={4.5} fill={o.color} stroke="white" strokeWidth={1.5} />;
              })}
            </g>
          </g>
          {data.magnetActive && (
            <circle cx={HALF} cy={HALF} r={data.magnetRadiusWorld * scale} fill="none" stroke="rgba(248,113,113,0.45)" strokeWidth={1.25} strokeDasharray="4 3" />
          )}
          {data.vacuumActive && (
            <circle cx={HALF} cy={HALF} r={data.collectRadiusWorld * scale} fill="none" stroke="rgba(167,139,250,0.5)" strokeWidth={1} />
          )}
          <polygon
            points={`${HALF},${HALF - 7} ${HALF - 5},${HALF + 4} ${HALF + 5},${HALF + 4}`}
            fill={godMode ? "#c4b5fd" : "#34d399"} stroke="white" strokeWidth={1.5} opacity={0.95}
          />
          <circle cx={HALF} cy={HALF} r={2} fill="white" />
          {data.speedActive && (
            <text x={HALF} y={HALF + 14} textAnchor="middle" fontSize="11">⚡</text>
          )}
        </svg>
        <div className="absolute bottom-1 right-1 flex gap-0.5 pointer-events-auto">
          {([0, 1, 2] as const).map(i => (
            <button
              key={i}
              type="button"
              onClick={() => setZoomIdx(i)}
              className={`w-6 h-6 text-[10px] font-bold rounded-md border ${zoomIdx === i ? "bg-emerald-500/90 border-emerald-200 text-white" : "bg-black/50 border-white/20 text-white/70"}`}
            >
              {i === 0 ? "−" : i === 1 ? "1×" : "+"}
            </button>
          ))}
      ...
      <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-0.5 justify-center text-[9px] text-white/75 font-semibold max-w-[260px]">
        <span className="flex items-center gap-1"><span className="inline-block w-2 h-2 rounded-full bg-amber-400" />Trash</span>
        <span className="flex items-center gap-1"><span className="inline-block w-2 h-2 rounded-full bg-red-500" />Magnet</span>
        <span className="flex items-center gap-1"><span className="inline-block w-2 h-2 rounded-full bg-yellow-400" />Speed</span>
        <span className="flex items-center gap-1"><span className="inline-block w-2 h-2 rounded-full bg-violet-400" />Bonus</span>
        {data.others.length > 0 && <span className="flex items-center gap-1"><span className="inline-block w-2 h-2 rounded-full bg-sky-400" />Players</span>}
      </div>
    </div>
  );
}
...
function App() {
  ...
  // ESC key — close admin console, overlays, then pause
  useEffect(() => {
  ...
  }, [state.status, state.isPaused, showSettings, showOwnerPanel, showOwnerLogin, showStats, showAdminConsole]);

  // Hidden admin console: Ctrl+Shift+` (Backquote)
  useEffect(() => {
    const inGame = state.status === "playing" || state.status === "waiting";
    const onToggle = (e: KeyboardEvent) => {
      if (!inGame || state.isPaused) return;
      if (e.ctrlKey && e.shiftKey && (e.key === "`" || e.code === "Backquote")) {
        e.preventDefault();
        setShowAdminConsole(v => !v);
      }
    };
  ...
  return (
    <div className="relative w-full h-full">
      ...
      <AdminCommandConsole
        ...
        onSubmit={code => gameRef.current?.submitAdminConsole(code)}
        lastStatus={state.ownerStatus}
      />
      ...
      {/* Global Chat Box */}
      {inGame && (
        <div className="absolute bottom-32 left-4 z-10 w-80 pointer-events-auto flex flex-col gap-2">
          <div className="bg-black/40 backdrop-blur-md rounded-xl p-3 h-48 overflow-y-auto flex flex-col gap-1 border border-white/10 shadow-lg">
            {state.chatMessages.map(msg => (
              <div key={msg.id} className="text-sm shadow-sm break-words">
                {msg.isSystem ? (
                  <span className="text-yellow-400 font-bold">[{msg.name}]: {msg.text}</span>
                ) : (
                  <span><span className="text-emerald-400 font-bold">{msg.name}:</span> <span className="text-white">{msg.text}</span></span>
                )}
              </div>
            ))}
            <div ref={chatEndRef} />
          ...
          <form onSubmit={(e) => {
            e.preventDefault();
            if (chatInput.trim()) {
              gameRef.current?.sendChat(chatInput);
              setChatInput("");
            }
            ...
            <input 
              ...
              placeholder="Chat or type a password..."
              className="flex-1 bg-black/50 backdrop-blur-md border border-white/20 rounded-lg px-3 py-2 text-sm text-white placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-emerald-500 shadow-md"
              onKeyDown={e => e.stopPropagation()} // Prevent WASD from triggering when typing
            />
          </form>
        </div>
      )}

      {/* Dynamic Bag Capacity Bar (Glassmorphism) */}
      {state.status === "playing" && (
        <div className="absolute bottom-20 left-1/2 -translate-x-1/2 z-10 w-72 pointer-events-none">
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-2 shadow-2xl">
            <div className="flex items-center justify-between text-[11px] uppercase tracking-wider text-white font-bold mb-1.5 px-2 drop-shadow-md">
              <span>🎒 Bag Capacity</span>
              <span>{state.collected} / {state.totalTrash}</span>
            </div>
            <div className="h-4 bg-black/40 rounded-full overflow-hidden border border-white/10 p-0.5">
              <div 
                className="h-full rounded-full transition-all duration-300 shadow-[0_0_10px_rgba(57,255,20,0.8)]"
                style={{ 
                  width: `${Math.max(3, (state.collected / Math.max(1, state.totalTrash)) * 100)}%`,
      ...
      {showOwnerLogin && (
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowOwnerLogin(false)}>
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-[90%]" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-gray-600 mb-3">Validated on the server (same as <span className="font-mono text-purple-800">Ctrl+Shift+`</span> console): <span className="font-mono">Slayers</span>, <span className="font-mono">PVP_PRO</span> (Overlord), <span className="font-mono">NordenMC</span>.</p>
            <input type="password" value={ownerPwInput} onChange={e => setOwnerPwInput(e.target.value)}
              ...
              placeholder="Slayers · PVP_PRO · NordenMC" autoFocus
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-6 max-w-lg w-[92%] max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-xl font-bold text-purple-900">
                {state.adminTier === "overlord" ? "⚡ Overlord Panel" : state.adminTier === "norden" ? "🛡️ Norden Panel" : "👑 Owner Panel"}
              </h2>
            ...
            {state.adminTier === "overlord" && (
              <div className="mb-4 p-3 rounded-xl bg-gradient-to-br from-violet-900/90 to-purple-900/85 text-white border border-violet-400/40">
                <div className="text-xs uppercase tracking-wide font-bold text-violet-200 mb-2">Overlord — server</div>
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <button type="button" onClick={() => sendAdmin({ kind: "freezeTimer" })} className="bg-white/15 hover:bg-white/25 text-sm font-semibold py-2 rounded-lg border border-white/20">Freeze timer</button>
                  <button type="button" onClick={() => sendAdmin({ kind: "unfreezeTimer" })} className="bg-white/15 hover:bg-white/25 text-sm font-semibold py-2 rounded-lg border border-white/20">Unfreeze timer</button>
                  <button type="button" onClick={() => sendAdmin({ kind: "clearUncollectedTrash" })} className="bg-amber-500/90 hover:bg-amber-400 text-amber-950 text-sm font-semibold py-2 rounded-lg col-span-2">Clear all litter (synced)</button>
                  <button type="button" onClick={() => sendAdmin({ kind: "resetScores" })} className="bg-rose-600/90 hover:bg-rose-500 text-sm font-semibold py-2 rounded-lg">Reset scores</button>
                  <button type="button" onClick={() => sendAdmin({ kind: "spawnPowerUpBurst", count: 8 })} className="bg-fuchsia-600/90 hover:bg-fuchsia-500 text-sm font-semibold py-2 rounded-lg">Power-up burst ×8</button>
                </div>
                <div className="text-xs uppercase tracking-wide font-bold text-violet-200 mb-2 mt-3">Overlord — local (your client)</div>
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <button type="button" onClick={() => sendAdmin({ kind: "setInfiniteStorage", enabled: !state.infiniteStorage })} className="bg-white/10 hover:bg-white/20 py-2 rounded-lg border border-white/15">
                    {state.infiniteStorage ? "∞ Bag: ON" : "∞ Bag: OFF"}
                  </button>
                  <button type="button" onClick={() => sendAdmin({ kind: "enableExecutionerVacuum", enabled: !state.executionerVacuum })} className="bg-white/10 hover:bg-white/20 py-2 rounded-lg border border-white/15">
                    {state.executionerVacuum ? "Vacuum: ON" : "Vacuum: OFF"}
                  </button>
                  <button type="button" onClick={() => sendAdmin({ kind: "setSpeedMultiplier", multiplier: state.speedMultiplier >= 2 ? 1 : 2 })} className="bg-white/10 hover:bg-white/20 py-2 rounded-lg border border-white/15">
                    Speed ×{state.speedMultiplier >= 2 ? "1" : "2"}
                  </button>
                  <button type="button" onClick={() => sendAdmin({ kind: "setCoinMultiplier", multiplier: state.coinMultiplier >= 2 ? 1 : 2 })} className="bg-white/10 hover:bg-white/20 py-2 rounded-lg border border-white/15">
                    Coins ×{state.coinMultiplier >= 2 ? "1" : "2"}
                  </button>
                  <button type="button" onClick={() => sendAdmin({ kind: "teleportAll", x: 0, z: 0 })} className="bg-white/10 hover:bg-white/20 py-2 rounded-lg border border-white/15 col-span-2">Teleport me to center</button>
                  <button type="button" onClick={() => sendAdmin({ kind: "clearAllTrash" })} className="bg-white/10 hover:bg-white/20 py-2 rounded-lg border border-white/15">Clear local trash view</button>
                  <button type="button" onClick={() => sendAdmin({ kind: "spawnMinkDrones", count: state.minkDrones > 0 ? 0 : 3 })} className="bg-white/10 hover:bg-white/20 py-2 rounded-lg border border-white/15">
                    Drones: {state.minkDrones > 0 ? "Off" : "×3"}
                  </button>
                </div>
              </div>
            )}
                  ...
                  {showPrivate && (
                    <>
                      ...
                      <div className="grid grid-cols-4 gap-2 mb-3">
                        {(["speedrun", "percent100", "powerup", "simulator"] as const).map(m => (
                ...
                <div className="grid grid-cols-2 gap-3">
                  {ALL_LEVELS.map(lvl => {
                    const desc: Record<Level, string> = {
                      ...
                      cybercity: "Futuristic neon metropolis with holograms",
                    };
                    return (
